...
import (
	...
	samplecrdv1 "github.com/zhiyxu/k8s-crd/pkg/apis/samplecrd/v1"
	clientset "github.com/zhiyxu/k8s-crd/pkg/client/clientset/versioned"
	networkscheme "github.com/zhiyxu/k8s-crd/pkg/client/clientset/versioned/scheme"
	informers "github.com/zhiyxu/k8s-crd/pkg/client/informers/externalversions/samplecrd/v1"
	listers "github.com/zhiyxu/k8s-crd/pkg/client/listers/samplecrd/v1"
)
...
func NewController(
	...
	networkInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		// Informer为每一种增量注册不同的EventHandler
		// 但实际上所有函数的结果都是将该事件对应的API对象加入到WorkQueue中
		// 需要注意的是，实际入队的并不是API对象本身，而是它们的Key，即该API对象的<namespace>/<key>
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	// 然后直接通过goroutine启动一个或并发启动多个“无限循环”任务
	// 这个“无限循环”任务的每一个循环周期，执行的是我们真正关心的业务逻辑
...
func (c *Controller) syncHandler(key string) error {
	...
	// 访问本地缓存的索引，获取这个key对应的Network对象
	// 这也是控制器从各种Lister中获取API资源对象的方式，如podLister、nodeLister
